...
const getSurvivalColor = (pct: number) =>
  pct >= 90 ? "text-lime-400" : pct >= 50 ? "text-yellow-400" : "text-red-400";
...
JOB_ORDER.slice(0, 14).forEach((j) => {
  JOB_LINE_OF[j] = "전사";
});
JOB_ORDER.slice(14, 28).forEach((j) => {
  JOB_LINE_OF[j] = "로그";
});
JOB_ORDER.slice(28).forEach((j) => {
  JOB_LINE_OF[j] = "주문술사";
});
...
export default function SavedResults({ onLoadSimulation, refreshKey }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!cancelled) {
        setRegionOrderMap(regMap);
        setSubAreaOrderMap(subMap);
      }
    })();
    return () => {
      cancelled = true;
  ...
  const resolveHs = useCallback(
    (s: SavedSimulationSummary, hs: SavedSimulationSummary["heroSummaries"][number]) => {
    ...
    >
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            {importPreview?.length || 0}개의 저장 결과가 포함된 파일입니다. 시뮬레이션을 다시 실행하지 않고 내 결과
            카드로 바로 저장합니다.
          </AlertDialogDescription>
        </AlertDialogHeader>
    ...
    <div className="saved-results-root space-y-3">
      ...
      <div className="flex items-center flex-wrap gap-2">
        ...
        <Select
          ...
          onValueChange={(v) => {
            setFilterQuestType(v);
            setFilterRegion("__all__");
            setFilterSubArea("__all__");
          }}
        >
        ...
        <div className="flex items-center gap-1.5">
          ...
          {!editMode ? (
            ...
            <>
              ...
              >
                <CheckSquare
                  className={`w-4 h-4 ${selectedIds.size === visible.length ? "text-primary" : "text-muted-foreground"}`}
                />
              </button>
              <Button
                variant="destructive"
      ...
      <div className="space-y-3">
            ...
            >
              <div className="flex items-stretch min-h-[170px]">
                ...
                <div className="flex items-center justify-center w-10 shrink-0 bg-primary border-r border-border/30">
                  <span className="text-sm font-bold font-mono !text-white" style={{ color: "#fff" }}>
                    {simIndex + 1}
                  </span>
                </div>
                ...
                <div className="flex-1 min-w-0 flex flex-col">
                  ...
                  <div className="saved-result-header px-3 py-2 flex items-center gap-4 flex-wrap">
                    <span
                      className={`saved-chip saved-chip-quest-type ${QUEST_TYPE_CHIP_STYLE[sim.questTypeKey] || "text-primary border-primary"}`}
                    >
                      {questTypeLabel}
                    </span>
                  ...
                  <div className="flex-1 grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-x-8 px-3 py-3">
                    ...
                    <div className="grid grid-cols-5 gap-5 min-w-0">
                      {Array.from({ length: 5 }).map((_, heroSlotIndex) => {
                        ...
                        const hero =
                          allHeroes.find((h) => h.id === hs.heroId) ||
                        ...
                        return (
                          <div key={hs.heroId} className="flex flex-col gap-1 min-w-0">
                            ...
                            <div className="flex items-center gap-1.5 min-w-0">
                              ...
                              <div className="text-[14px] font-bold text-foreground truncate min-w-0">
                                {hs.heroName}
                              </div>
                            </div>
                            ...
                            <div className="flex items-center justify-between gap-2">
                              <div className="flex items-center gap-1">
                                ...
                                <span
                                  className={`text-[12px] font-mono font-bold tabular-nums ${getSurvivalColor(hs.survivalRate)}`}
                                >
                                      ...
                                      <span key={e.element} className="flex items-center gap-0.5">
                                        {e.iconPath && (
                                          <img
                                            ...
                                            onError={(ev) => {
                                              if (!ev.currentTarget.src.endsWith("/images/fallback.svg"))
                                                ev.currentTarget.src = "/images/fallback.svg";
                                            }}
                                          />
                                        )}
                                        <span
                                          className={`text-[13px] font-mono font-bold tabular-nums text-black dark:text-white`}
                                        >
                            ...
                            <div className="flex items-center gap-1 text-[13px]">
                              ...
                              <div className="h-1.5 bg-secondary/50 rounded-full overflow-hidden flex-1 min-w-0">
                                <div
                                  ...
                                  style={{ width: `${Math.min(100, hs.damageShare)}%` }}
                                />
                              </div>
                              <span
                                className={`font-bold font-mono tabular-nums shrink-0 text-left ${getShareTextColor(hs.damageShare)}`}
                                style={{ width: "4ch" }}
                              >
                                {hs.damageShare.toFixed(0)}%
                              </span>
                            </div>
                            {/* Tank bar */}
                            <div className="flex items-center gap-1 text-[13px]">
                              ...
                              <div className="h-1.5 bg-secondary/50 rounded-full overflow-hidden flex-1 min-w-0">
                                <div
                                  ...
                                  style={{ width: `${Math.min(100, tankShare)}%` }}
                                />
                              </div>
                              <span
                                className={`font-bold font-mono tabular-nums shrink-0 text-left ${getShareTextColor(tankShare)}`}
                                style={{ width: "4ch" }}
                              >
                                {tankShare.toFixed(0)}%
                              </span>
                    ...
                    <div className="mr-3 flex items-center gap-9 shrink-0">
                      <div className="text-center">
                        ...
                        <div className={`text-2xl font-bold font-mono ${getWinRateColor(sim.winRate)}`}>
                          {(sim.winRate >= 100 ? 100 : Math.floor(sim.winRate * 10) / 10).toFixed(1)}%
                        </div>
                      </div>
                      <div className="text-center">
                        <div className="text-[11px] text-muted-foreground dark:text-foreground/80">장비 대비</div>
                        <div
                          className={`text-2xl font-bold font-mono ${gearRatio !== null ? getWinRateColor(gearRatio) : "text-muted-foreground/50"}`}
                        >
                  ...
                  <div className="border-t border-border/30 px-3 py-2 flex items-center gap-3 text-[13px] flex-wrap">
                      ...
                      <div className="flex items-center gap-1.5">
                        {sim.barrierInfos.map((b, i) => {
                          ...
                          return (
                            <span
                              key={i}
                              className={`saved-chip saved-chip-barrier text-[13px] ${isMet ? "is-met text-lime-700" : "is-unmet text-red-700"}`}
                            >
                              {b.iconPath && (
                                <img
                                  src={b.iconPath}
                    ...
                    {sim.barrierInfos?.length && sim.boosterImage ? (
                      <span className="text-muted-foreground/40">·</span>
                    ) : null}
                    {sim.boosterImage && (
                      <span className="flex items-center gap-1.5">
                        <img
                          src={sim.boosterImage}
                          alt="booster"
        ...
        {visible.length === 0 && (
          <div className="text-center py-10 text-xs text-muted-foreground">필터에 맞는 결과가 없습니다.</div>
        )}
      </div>

      <div className="text-center text-[10px] text-muted-foreground/50 pt-2">
        ℹ️ 카드를 누르면 파티 상세를 볼 수 있습니다. 파일 불러오기는 저장된 결과를 카드로 바로 추가합니다.
        <br />
      ...
      <AlertDialog open={!!extractTarget} onOpenChange={(o) => !o && setExtractTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              이 결과를 어떻게 내보낼까요? 클립보드 복사는 커뮤니티 공유용 코드 붙여넣기에 적합하고, 파일 저장은 개별
              백업에 적합합니다.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <div className="flex flex-col gap-2 py-2">
            <Button
              ...
              onClick={() => extractTarget && handleExtractCopy(extractTarget)}
            >
            ...
            <Button
              variant="outline"
              className="justify-start gap-2"
              onClick={() => extractTarget && handleExtractFile(extractTarget)}
            >
