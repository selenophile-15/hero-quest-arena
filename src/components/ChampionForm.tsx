...
import {
  loadFamiliars,
  loadAurasongs,
  getAurasongSkillEffect,
  getAurasongSkillIconPath,
  ensureAurasongDataLoaded,
  getFamiliarImagePath,
  getAurasongImagePath,
  getLeaderSkillTierName,
} from "@/lib/championEquipUtils";
...
export default function ChampionForm({
  ...
  const [promoted, setPromoted] = useState(hero?.promoted || false);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const [familiars, aurasongs] = await Promise.all([loadFamiliars(), loadAurasongs()]);
      setFamiliarItems(familiars);
      setAurasongItems(aurasongs);

      function collectEnchantImgs(items: typeof familiars): string[] {
        const imgs: string[] = [];
        items.forEach((item) => {
      ...
      // 퍼밀리어(현재 첫 슬롯)를 먼저 로드 후 표시
      await preloadImagesAndWait([...familiars.map((i) => i.imagePath), ...collectEnchantImgs(familiars)]);
      setEquipImagesReady(true);

      // 오라의 노래는 백그라운드 로드
      preloadImagesAndWait([...aurasongs.map((i) => i.imagePath), ...collectEnchantImgs(aurasongs)]);
    };
    load();
  }, []);
  ...
  const handleNumericChange =
    (setter: (v: number | "") => void, max?: number) => (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const renderEquipSlot = (slotIdx: number) => {
    ...
    const displayElement =
      slotData?.element ||
      (equipItem?.uniqueElement?.length
        ? { type: equipItem.uniqueElement[0], tier: equipItem.uniqueElementTier || 1, affinity: true }
        : null);
    const displaySpirit =
      slotData?.spirit ||
      (equipItem?.uniqueSpirit?.length ? { name: equipItem.uniqueSpirit[0], affinity: true } : null);
        ...
        >
          <div className="w-full flex items-center justify-between gap-1 px-1.5 pt-1">
            ...
            <div className="flex items-center gap-1">
              {equipItem && (
                <span className="text-xs font-bold text-foreground/90 bg-background/80 rounded border border-border/40 px-1 py-0.5">
                  T{equipItem.tier}
                </span>
              )}
              {equipItem?.relic && (
          ...
          <div className="grid grid-cols-3 gap-0.5 w-[90%] p-0.5 mb-0.5 self-center">
            <div
              ...
              onClick={(e) => {
                e.stopPropagation();
                setEnchantInitialTab("element");
                setEnchantDialogOpen(true);
              }}
            >
              {displayElement ? (
                <img
          ...
          <div className="mx-1 mt-1 mb-1 rounded bg-card/80 border border-border/40 py-1 text-center">
            <p
              className={`text-sm truncate leading-tight font-bold px-1 ${equipItem ? "text-foreground" : "text-muted-foreground"}`}
            >
              {equipItem?.name || "-"}
            </p>
          </div>
          ...
          <div className="flex flex-1 min-h-0 gap-4">
            ...
            <div className="flex-1 flex flex-col min-h-0">
              ...
              <div className="flex items-center gap-1 mb-1">
                <button
                  onClick={() => {
                    setEquipDialogType("familiar");
                    setChampionManualMode(!!equipmentSlots[0]?.item?.manual);
                  }}
                ...
                <button
                  onClick={() => {
                    setEquipDialogType("aurasong");
                    setChampionManualMode(!!equipmentSlots[1]?.item?.manual);
                  }}
                ...
                {championManualMode ? (
                  <div className="flex items-center gap-1.5 ml-auto">
                    <Button
                      ...
                      onClick={() => setChampionManualMode(false)}
                    >
                      취소
                    </Button>
                    <Button
                      size="sm"
                      className="h-7 text-xs"
                      onClick={() => championManualFormRef.current?.triggerConfirm()}
                    >
                      적용
                    </Button>
                  </div>
                  ...
                  <>
                    ...
                    <div className="flex flex-col gap-1 flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={() => {
                            setEquipFilterStat("_all");
                            setEquipFilterElement("_all");
                            setEquipFilterSpirit("_all");
                            setChampSortKey("");
                            setChampSortDir("asc");
                          }}
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          size="sm"
                          variant="outline"
                          className="h-7 text-xs bg-primary/30 border-primary/40 text-white hover:bg-primary/50"
                          onClick={() => {
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              setEquipmentSlots((prev) =>
                                prev.map((s, i) => (i === slotIdx ? { ...s, starforged: v } : s)),
                              );
                            }}
                        ...
                        >
                          <button
                            onClick={() => setChampViewMode("album")}
                            className={`flex-1 px-1.5 py-1 ${champViewMode === "album" ? "bg-primary text-primary-foreground" : "bg-secondary/40 text-muted-foreground hover:bg-secondary/60"}`}
                            title="앨범"
                          >
                            <LayoutGrid className="w-3.5 h-3.5 mx-auto" />
                          </button>
                          <button
                            onClick={() => setChampViewMode("table")}
                            className={`flex-1 px-1.5 py-1 ${champViewMode === "table" ? "bg-primary text-primary-foreground" : "bg-secondary/40 text-muted-foreground hover:bg-secondary/60"}`}
                            title="테이블"
                          >
                  ...
                  {!equipImagesReady ? (
                    <div className="flex flex-col items-center justify-center py-16 gap-4">
                      <div className="relative w-12 h-12">
                        <div
                          className="absolute inset-0 border-[3px] border-dashed border-primary/30 rounded-full animate-spin"
                          style={{ animationDuration: "2s" }}
                        />
                        <div
                          className="absolute inset-1 border-[3px] border-transparent border-t-primary rounded-full animate-spin"
                    ...
                    <ManualEquipmentForm
                      ...
                      onConfirm={(item) => {
                        ...
                        if (item.uniqueElement?.length) {
                          slotElement = {
                            type: item.uniqueElement[0],
                            tier: item.uniqueElementTier || 4,
                            affinity: true,
                          };
                        }
                        ...
                        <thead className="sticky top-0 bg-background z-10">
                          <tr className="border-b-2 border-border">
                            {[
                              ...
                              <th
                                key={ci}
                                className={`text-center py-2 px-1.5 ${col.color} font-semibold ${col.key ? "cursor-pointer hover:text-primary" : ""}`}
                                onClick={() =>
                                  col.key &&
                                  (() => {
                                    if (champSortKey === col.key)
                                      setChampSortDir((d) => (d === "asc" ? "desc" : "asc"));
                                    else {
                                      setChampSortKey(col.key);
                                      setChampSortDir("desc");
                                    }
                                  })()
                                }
                              >
                          ...
                          {(() => {
                            ...
                            if (champSortKey) {
                              sorted.sort((a, b) => {
                                ...
                                if (champSortKey === "name") {
                                  av = a.name;
                                  bv = b.name;
                                } else if (champSortKey === "tier") {
                                  av = a.tier;
                                  bv = b.tier;
                                } else if (champSortKey === "atk") {
                                  av = a.stats.find((s) => s.key === "장비_공격력")?.value || 0;
                                  bv = b.stats.find((s) => s.key === "장비_공격력")?.value || 0;
                                } else if (champSortKey === "def") {
                                  av = a.stats.find((s) => s.key === "장비_방어력")?.value || 0;
                                  bv = b.stats.find((s) => s.key === "장비_방어력")?.value || 0;
                                } else if (champSortKey === "hp") {
                                  av = a.stats.find((s) => s.key === "장비_체력")?.value || 0;
                                  bv = b.stats.find((s) => s.key === "장비_체력")?.value || 0;
                                } else if (champSortKey === "crit") {
                                  av = a.stats.find((s) => s.key === "장비_치명타확률%")?.value || 0;
                                  bv = b.stats.find((s) => s.key === "장비_치명타확률%")?.value || 0;
                                } else if (champSortKey === "eva") {
                                  av = a.stats.find((s) => s.key === "장비_회피%")?.value || 0;
                                  bv = b.stats.find((s) => s.key === "장비_회피%")?.value || 0;
                                }
                                if (typeof av === "string" && typeof bv === "string")
                                  return champSortDir === "asc" ? av.localeCompare(bv) : bv.localeCompare(av);
                                return champSortDir === "asc"
                            ...
                            return (
                                ...
                                onClick={() => {
                                  ...
                                  if (isSelected) {
                                    ...
                                    if (item.uniqueElement?.length) {
                                      ...
                                      if (existing) {
                                        const hasAff =
                                          item.elementAffinity?.includes(existing.type) ||
                                          item.elementAffinity?.includes("모든 원소");
                              ...
                              >
                                ...
                                <td
                                  className={`py-1.5 px-1.5 text-center tabular-nums ${atkVal ? "text-red-400" : "text-muted-foreground/30"}`}
                                >
                                  {atkVal ? formatNumber(atkVal) : "-"}
                                </td>
                                <td
                                  className={`py-1.5 px-1.5 text-center tabular-nums ${defVal ? "text-blue-400" : "text-muted-foreground/30"}`}
                                >
                                  {defVal ? formatNumber(defVal) : "-"}
                                </td>
                                <td
                                  className={`py-1.5 px-1.5 text-center tabular-nums ${hpVal ? "text-orange-400" : "text-muted-foreground/30"}`}
                                >
                                  {hpVal ? formatNumber(hpVal) : "-"}
                                </td>
                                <td
                                  className={`py-1.5 px-1 text-center tabular-nums ${critVal ? "text-yellow-400" : "text-muted-foreground/30"}`}
                                >
                                  {critVal ? `${critVal}%` : "-"}
                                </td>
                                <td
                                  className={`py-1.5 px-1 text-center tabular-nums ${evaVal ? "text-teal-400" : "text-muted-foreground/30"}`}
                                >
                                  {evaVal ? `${evaVal}%` : "-"}
                                </td>
                                    ...
                                    <span>
                                      {item.elementAffinity.map((el, i) => (
                                        <span key={el}>
                                          ...
                                          <span className={ELEMENT_COLORS_LOCAL[el] || "text-foreground"}>
                                            {el === "모든 원소" ? "모든 원소" : el}
                                          </span>
                                        </span>
                                      ))}
                                  ...
                                  {item.uniqueElement?.length ? (
                                    <span>
                                      {item.uniqueElement.map((el) => (
                                        <span key={el} className={`${ELEMENT_COLORS_LOCAL[el] || ""} font-semibold`}>
                                          {el} (T{item.uniqueElementTier})
                                        </span>
                                      ))}
                                    </span>
                                  ) : item.uniqueSpirit?.length ? (
                                    <span className="text-purple-300 font-semibold">
                                      {item.uniqueSpirit.join(", ")}
                                    </span>
                                  ) : (
                                    <span className="text-muted-foreground/30">-</span>
                                  )}
                              ...
                              <TooltipTrigger asChild>
                                <button
                                      ...
                                      if (item.uniqueElement?.length) {
                                        ...
                                        if (existing) {
                                          const hasAff =
                                            item.elementAffinity?.includes(existing.type) ||
                                            item.elementAffinity?.includes("모든 원소");
                                          const allElAff = isAllElementAffinityOnly(
                                  ...
                                  className={`relative flex flex-col rounded-lg border-[2.5px] transition-all cursor-pointer aspect-square overflow-hidden ${
                                    isSelected
                                      ? `${QUALITY_BORDER[quality]} bg-accent/10`
                                      ...
                                      (() => {
                                        ...
                                        return (
                                          <>
                                            {hasElems && (
                                              <div
                                                className={`flex items-center gap-0.5 ${!showDivider ? "mx-auto" : ""}`}
                                              >
                                                {elemAffs.map((el) => (
                                                  <img
                                                    key={el}
                                                    src={`/images/elements/${ELEMENT_ENG_MAP[el] || el}.webp`}
                                                    alt={el}
                                                    className="w-6 h-6"
                                                    loading="eager"
                                                    decoding="sync"
                                              ...
                                              >
                                                {spiritAffs.map((sp) => {
                                                  ...
                                                  return eng ? (
                                                    <img
                                                      key={sp}
                                                      src={`/images/enchant/spirit/${eng}_1.webp`}
                                                      alt={sp}
                                                      className="w-6 h-6"
                                                      loading="eager"
                                                      decoding="sync"
                                                ...
                                                {uniqueSp.map((sp) => {
                                                  ...
                                                  const imgSrc =
                                                    sp === "문드라"
                                                      ? "/images/enchant/spirit/mundra.webp"
                                                      : eng
                                                        ? `/images/enchant/spirit/${eng}_2.webp`
                                                        : "";
                                                  return (
                                                    <img
                                                      key={`us-${sp}`}
                              ...
                              >
                                <p className="font-bold text-sm">
                                  ...
                                  <span className="text-muted-foreground font-normal">
                                    (T{item.tier}, {item.typeKor || item.type})
                                  </span>
                                </p>
                                {item.stats.length > 0 && (
                                ...
                                {item.elementAffinity && item.elementAffinity.length > 0 && (
                                  <div className="text-xs">
                                    ...
                                    {item.elementAffinity.map((el: string, ei: number) => (
                                      <span
                                        key={ei}
                                        className={`font-semibold ${ELEMENT_COLORS_LOCAL[el] || "text-foreground"}`}
                                      >
                                        {ei > 0 ? ", " : ""}
                                        {el}
                                      </span>
                                ...
                                {item.spiritAffinity && item.spiritAffinity.length > 0 && (
                                  <div className="text-xs">
                                    ...
                                    {item.spiritAffinity.map((sp: string, si: number) => (
                                      <span key={si} className="text-foreground font-semibold">
                                        {si > 0 ? ", " : ""}
                                        {sp} (T{getSpiritTier(sp)})
                                      </span>
                                    ))}
                                  </div>
                                )}
                                  ...
                                  <div className="text-xs">
                                    ...
                                    {item.uniqueElement.map((el: string, i: number) => (
                                      <span
                                        ...
                                        className={`font-semibold ${ELEMENT_COLORS_LOCAL[el] || "text-foreground"}`}
                                      >
                                        {el} (T{item.uniqueElementTier || 1})
                                        {i < item.uniqueElement!.length - 1 ? ", " : ""}
                                      </span>
                                    ))}
                                  </div>
                                )}
                                  ...
                                  <div className="text-xs">
                                    ...
                                    {item.uniqueSpirit.map((sp: string, i: number) => (
                                      <span key={sp} className="text-foreground font-semibold">
                                        {sp} (T{getSpiritTier(sp)}){i < item.uniqueSpirit!.length - 1 ? ", " : ""}
                                      </span>
                                    ))}
                                  </div>
                                )}
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-stretch gap-0 rounded border-[2.5px] cursor-pointer transition-all overflow-hidden ${
                        (i === 0 && equipDialogType === "familiar") || (i === 1 && equipDialogType === "aurasong")
                          ? "border-primary ring-1 ring-primary/30"
                          : s.item
                            ? QUALITY_BORDER[s.quality]
                    ...
                    >
                      ...
                      <div
                        ...
                        style={{
                          background: s.item
                            ? champQualityColor[s.quality] || "transparent"
                            : "hsl(var(--secondary) / 0.3)",
                        }}
                      >
                        <span
                          className={`text-[10px] font-bold ${s.starforged ? "starforged-slot-number relative z-10" : "text-black"}`}
                        >
                          {i + 1}
                        </span>
                      </div>
                        ...
                        <div className="flex items-center gap-1.5 flex-1 min-w-0 p-1.5">
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="flex items-center gap-1 mt-0.5 flex-wrap">
                              {s.element && (
                                <span
                                  className={`text-[9px] font-bold ${ELEMENT_COLORS_LOCAL[s.element.type] || "text-foreground/70"}`}
                                >
                                  {hasAffinityEl && "★ "}
                              ...
                              {s.spirit && (
                                <span className={`text-[9px] font-bold text-purple-400`}>
                                  {hasAffinitySp && "★ "}
                                  {s.spirit.name} (T{getSpiritTier(s.spirit.name)})
                                </span>
                              )}
                            </div>
                          </div>
    ...
    <div className="animate-fade-in">
      ...
      <div className="sticky top-14 z-10 bg-card/90 backdrop-blur-sm border-b border-border py-2 -mx-6 px-6 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button
            ...
            disabled={!champCalcResult}
            className="btn-force-white gap-1.5"
          >
          ...
          {hero && onSaveAs && (
            <Button type="button" variant="outline" size="sm" onClick={handleSaveAs}>
      ...
      <div className="space-y-4 mt-4" ref={formRef} onKeyDown={handleKeyDown}>
          ...
          <div className="grid grid-cols-[1fr_1fr_auto_0.6fr_0.4fr_0.5fr_0.8fr_0.8fr] gap-3 items-end">
            ...
            <div>
              ...
              <Input
                ...
                placeholder="이름 입력"
                className={`h-9 text-sm ${nameError ? "border-red-500 ring-1 ring-red-500" : ""}`}
              />
            ...
            <div>
              ...
              <Input
                type="number"
                value={rank}
                onChange={handleNumericChange(setRank as any, championMaxRank)}
                min={1}
                max={championMaxRank}
            ...
            <div>
              ...
              <Input
                type="number"
                value={level}
                onChange={handleNumericChange(setLevel as any, 50)}
                min={1}
                max={50}
                className="h-9 text-sm"
              />
          ...
          <div className="card-fantasy p-3">
            ...
            <div className="space-y-1.5">
              ...
              {[
                ...
                {
                  icon: STAT_ICON_MAP.critDmg,
                  value: champCalcResult?.totalCritDmg ?? critDmg,
                  suffix: "",
                  isCritDmg: true,
                  baseKey: "critDmg",
                },
                {
                  icon: STAT_ICON_MAP.critAttack,
                  value: champCalcResult?.critAttack ?? critAttack,
                  suffix: "",
                  baseKey: "critAttack",
                },
                {
                  icon: STAT_ICON_MAP.evasion,
                  value: champCalcResult?.totalEvasion ?? evasion,
                  suffix: " %",
                  baseKey: "evasion",
                },
                {
                  icon: STAT_ICON_MAP.threat,
                  value: champCalcResult?.totalThreat ?? threat,
                  suffix: "",
                  baseKey: "threat",
                },
              ].map((stat, i) => {
                ...
                return (
                  <div key={i} className="flex items-center gap-2 py-0.5 px-1">
                    ...
                    <span className="text-sm text-foreground ml-auto tabular-nums">
                      {stat.value
                        ? (stat as any).isCritDmg
                          ? `x${(Number(stat.value) / 100).toFixed(1)}`
                          : `${formatNumber(stat.value)}${stat.suffix}`
                        : "-"}
                    </span>
                    {diff !== null && diff !== 0 && (
                      <span
                        className={`text-[10px] font-semibold tabular-nums ml-1 ${diff > 0 ? "text-green-400" : "text-red-400"}`}
                      >
                        {diff > 0 ? "▲" : "▼"}
                        {(stat as any).isCritDmg
                          ? (Math.abs(diff) / 100).toFixed(1)
                          : formatNumber(Math.abs(Math.round(diff)))}
                      </span>
                    )}
                  </div>
                );
              ...
              <div className="flex items-center gap-2 py-0.5 px-1">
                ...
                <span className="text-sm text-foreground ml-auto tabular-nums">
                  {totalEquipElement > 0
                    ? formatNumber(totalEquipElement)
                    : elementValue
                      ? formatNumber(elementValue)
                      : "-"}
                </span>
              </div>
              <div className="flex items-center gap-2 py-0.5 px-1">
                ...
                <div className="border border-border rounded overflow-hidden">
                  ...
                  <div className="grid grid-cols-[50px_44px_1fr_2fr] gap-0 border-b border-border/50">
                    <div className="px-1 py-1.5 flex items-center justify-center">
                      <span
                        ...
                        style={{ backgroundColor: "#c02020", color: "white" }}
                      >
                        챔피언
                      </span>
                    </div>
                    ...
                    <div className="px-1 py-1.5 flex items-center justify-center text-xs text-foreground">
                      {getLeaderSkillTierName(championName, leaderSkillTier)}
                    </div>
                  ...
                  <div className="grid grid-cols-[50px_44px_1fr_2fr] gap-0">
                    <div className="px-1 py-1.5 flex items-center justify-center">
                      <span
                        className="px-1 py-0.5 rounded text-[10px] font-bold"
                        style={{ backgroundColor: "#4a7c3f", color: "white" }}
                      >
                        오라
                      </span>
                    </div>
        ...
        <EnchantPickerDialog
          ...
          itemInfoPerSlot={equipmentSlots.map((s) =>
            s.item
              ? {
            ...
            <AlertDialogContent>
              ...
              <AlertDialogFooter>
                ...
                <AlertDialogAction
                  ...
                  className="bg-destructive text-white btn-force-white hover:bg-destructive/90"
                >
                  덮어쓰기
                </AlertDialogAction>
              </AlertDialogFooter>
