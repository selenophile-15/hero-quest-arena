...
export default function EquipmentSelectDialog({
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      preloadImages(items.map((item) => item.imagePath).filter(Boolean));
    };
    load();
  ...
  const handleSelectItem = useCallback(
    (item: EquipmentItem) => {
      ...
      if (!prevHadUniqueElement && newSlots[activeSlot]?.element) {
        userEnchantRef.current[activeSlot] = {
          ...userEnchantRef.current[activeSlot],
          element: newSlots[activeSlot].element,
        };
      }
      if (!prevHadUniqueSpirit && newSlots[activeSlot]?.spirit) {
        userEnchantRef.current[activeSlot] = {
          ...userEnchantRef.current[activeSlot],
          spirit: newSlots[activeSlot].spirit,
        };
      }
      ...
      if (item.uniqueElement?.length) {
        ...
        if (existing) {
          const hasAffinity =
            item.elementAffinity?.includes(existing.type) || item.elementAffinity?.includes("모든 원소");
  ...
  const handleClearSlot = useCallback(() => {
    ...
    newSlots[activeSlot] = {
      item: null,
      quality: newSlots[activeSlot]?.quality || "common",
      element: keepElement,
      spirit: keepSpirit,
    };
    setSlots(newSlots);
            ...
            <div className="flex items-center gap-2 px-1 text-xs mb-1">
              {manualMode ? (
                <div className="flex items-center gap-1.5 ml-auto">
                  <Button variant="outline" size="sm" className="h-7 text-xs" onClick={() => setManualMode(false)}>
                    취소
                  </Button>
                  <Button size="sm" className="h-7 text-xs" onClick={() => manualFormRef.current?.triggerConfirm()}>
                    적용
                  </Button>
                </div>
              ) : (
                <>
                  ...
                  <div className="flex flex-col gap-1 flex-1 min-w-0">
                    ...
                    {/* Sub-row 2: 등급 + 천상 + [앨범|테이블] on right */}
                    <div className="flex items-center gap-2">
                ...
                {manualMode ? (
                  <ManualEquipmentForm
                    ...
                    onConfirm={(item) => {
                      ...
                      // Save current user enchants before switching
                      if (!prevHadUniqueElement && existingSlot?.element) {
                        userEnchantRef.current[activeSlot] = {
                          ...userEnchantRef.current[activeSlot],
                          element: existingSlot.element,
                        };
                      }
                      if (!prevHadUniqueSpirit && existingSlot?.spirit) {
                        userEnchantRef.current[activeSlot] = {
                          ...userEnchantRef.current[activeSlot],
                          spirit: existingSlot.spirit,
                        };
                      }
                      ...
                      if (item.uniqueElement?.length) {
                        ...
                        const saved = userEnchantRef.current[activeSlot]?.element;
                        const existing = saved || slotElement;
                        if (existing) {
                          const hasAffinity =
                            item.elementAffinity?.includes(existing.type) ||
                            item.elementAffinity?.includes("모든 원소");
                          const allElAff = isAllElementAffinityOnly(item.elementAffinity, existing.type);
                          slotElement = { ...existing, affinity: !!hasAffinity, allElementAffinity: allElAff };
                        } else {
                          slotElement = null;
                        }
                      }
                      ...
                      if (item.uniqueSpirit?.length) {
                        ...
                        const saved = userEnchantRef.current[activeSlot]?.spirit;
                        const existing = saved || slotSpirit;
                        if (existing) {
                          const hasAffinity = item.spiritAffinity?.includes(existing.name);
                          slotSpirit = { ...existing, affinity: !!hasAffinity };
                        } else {
                          slotSpirit = null;
                        }
                      }
                    ...
                    <table className="w-full text-xs border-collapse" style={{ tableLayout: "fixed" }}>
                      ...
                      <thead className="sticky top-0 bg-background z-10">
                        <tr className="border-b-2 border-border">
                          <th
                            className="text-center py-2 px-2 text-foreground/60 font-semibold cursor-pointer hover:text-primary"
                            onClick={() => handleTableSort("name")}
                          >
                            이름 {tableSortKey === "name" && (tableSortDir === "asc" ? "▲" : "▼")}
                          </th>
                          <th
                            className="text-center py-2 px-1.5 text-foreground/60 font-semibold cursor-pointer hover:text-primary"
                            onClick={() => handleTableSort("type")}
                          >
                            타입 {tableSortKey === "type" && (tableSortDir === "asc" ? "▲" : "▼")}
                          </th>
                          <th
                            className="text-center py-2 px-1 text-foreground/60 font-semibold cursor-pointer hover:text-primary"
                            onClick={() => handleTableSort("tier")}
                          >
                            T {tableSortKey === "tier" && (tableSortDir === "asc" ? "▲" : "▼")}
                          </th>
                          <th
                            className="text-center py-2 px-1.5 text-red-400/80 font-semibold cursor-pointer hover:text-primary"
                            onClick={() => handleTableSort("atk")}
                          >
                            ATK {tableSortKey === "atk" && (tableSortDir === "asc" ? "▲" : "▼")}
                          </th>
                          <th
                            className="text-center py-2 px-1.5 text-blue-400/80 font-semibold cursor-pointer hover:text-primary"
                            onClick={() => handleTableSort("def")}
                          >
                            DEF {tableSortKey === "def" && (tableSortDir === "asc" ? "▲" : "▼")}
                          </th>
                          <th
                            className="text-center py-2 px-1.5 text-orange-400/80 font-semibold cursor-pointer hover:text-primary"
                            onClick={() => handleTableSort("hp")}
                          >
                            HP {tableSortKey === "hp" && (tableSortDir === "asc" ? "▲" : "▼")}
                          </th>
                          <th
                            className="text-center py-2 px-1 text-yellow-400/80 font-semibold cursor-pointer hover:text-primary"
                            onClick={() => handleTableSort("crit")}
                          >
                            CRIT.C {tableSortKey === "crit" && (tableSortDir === "asc" ? "▲" : "▼")}
                          </th>
                          <th
                            className="text-center py-2 px-1 text-teal-400/80 font-semibold cursor-pointer hover:text-primary"
                            onClick={() => handleTableSort("eva")}
                          >
                            EVA {tableSortKey === "eva" && (tableSortDir === "asc" ? "▲" : "▼")}
                          </th>
                        ...
                        {(() => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <td className="py-1.5 px-1.5 text-center text-foreground whitespace-nowrap">
                                {item.typeKor}
                              </td>
                                  ...
                                  <span>
                                    {item.elementAffinity.map((el, i) => (
                                      <span key={el}>
                                        ...
                                        <span className={ELEMENT_COLORS[el] || "text-foreground"}>
                                          {el === "모든 원소" ? "모든 원소" : el}
                                        </span>
                                      </span>
                                    ))}
                                ...
                                {item.uniqueElement?.length ? (
                                  <span>
                                    {item.uniqueElement.map((el) => (
                                      <span key={el} className={`${ELEMENT_COLORS[el] || ""} font-semibold`}>
                    ...
                    <div className="grid grid-cols-6 gap-3">
                      {filteredItems.map((item, idx) => {
                        ...
                        const hasAffinityIcons =
                          elemAffs.length > 0 || uniqueElems.length > 0 || spiritAffs.length > 0 || uniqueSp.length > 0;
                            ...
                            <TooltipTrigger asChild>
                              ...
                              >
                                ...
                                <div className="flex flex-col items-center w-full relative" style={{ height: "75%" }}>
                                  ...
                                  {isRelicBlocked && (
                                    <span className="absolute top-5 right-0 text-[7px] text-red-400 bg-background/80 rounded px-0.5 z-10">
                                      유물 중복
                                    </span>
                                  )}
                                      ...
                                      <img
                                        ...
                                        onError={(e) => {
                                          ...
                                          if (p) {
                                            ...
                                            s.className = "text-[9px] text-muted-foreground text-center";
                                            s.textContent = item.name.slice(0, 6);
                                            p.appendChild(s);
                                          }
                                        }}
                                      />
                            ...
                            >
                              <p className="font-bold text-sm">
                                ...
                                <span className="text-muted-foreground font-normal">
                                  (T{item.tier}, {item.typeKor})
                                </span>
                              </p>
                              {item.type === "dual_wield" && item.judgmentTypes?.length > 0 && (
                                <p className="text-xs text-muted-foreground">
                                  판정 타입:{" "}
                                  <span className="text-foreground font-medium">{item.judgmentTypes.join(", ")}</span>
                                </p>
                              )}
                              ...
                              {item.elementAffinity && item.elementAffinity.length > 0 && (
                                <div className="text-xs">
                                  ...
                                  {item.elementAffinity.map((el, i) => (
                                    <span
                                      key={el}
                                      className={`font-semibold ${ELEMENT_COLORS[el] || "text-foreground"}`}
                                    >
                                      {el}
                                      {i < item.elementAffinity!.length - 1 ? ", " : ""}
                                    </span>
                                  ))}
                                </div>
                              )}
                              ...
                              {item.airshipPower > 0 && (
                                <p className="text-xs text-muted-foreground">
                                  에어쉽파워: {formatNumber(item.airshipPower)}
                                </p>
                              )}
                            </TooltipContent>
                          </Tooltip>
              ...
              {slots.map((s, i) => {
                ...
                return (
                  <div
                    ...
                    style={
                      s.item ? { boxShadow: `0 0 4px ${QUALITY_RADIAL[s.quality]?.replace(/[\d.]+\)$/, "0.3)")}` } : {}
                    }
                  >
                    ...
                    <div
                      ...
                      style={{
                        background: s.item ? qualityColor[s.quality] || "transparent" : "hsl(var(--secondary) / 0.3)",
                      }}
                    >
                      <span
                      ...
                      <div className="flex items-center gap-1.5 flex-1 min-w-0 p-1.5">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="flex items-center gap-1 mt-0.5 flex-wrap">
                            {s.element && (
                              <span
                                className={`text-[9px] font-bold ${ELEMENT_COLORS[s.element.type] || "text-foreground/70"}`}
                              >
                                {hasAffinityEl && "★ "}
