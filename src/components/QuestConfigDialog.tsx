...
const DIFFICULTY_COLORS: Record<string, { bg: string; border: string; text: string; textLight: string }> = {
  쉬움: {
    bg: "bg-[hsl(80,40%,30%)]/20",
    border: "border-[hsl(80,50%,40%)]",
    text: "text-[hsl(80,50%,45%)]",
    textLight: "text-[hsl(80,45%,30%)]",
  },
...
export default function QuestConfigDialog({
  open,
  onOpenChange,
  questDataMap,
  questFiles,
  ...
  const areaCards = useMemo(() => {
    ...
    const cards = region.subAreas.map((sub, subIdx) => ({
      ...
      subAreaIdx: subIdx,
      quests:
        region.boss && questGroups.normal.length > 0
          ? questGroups.normal
  ...
  const stepTitle = step === "type" ? "퀘스트 종류" : step === "region" ? "지역 선택" : "세부 지역 & 난이도";
            ...
            <div className="flex flex-wrap justify-center gap-4">
              {areaCards.map((card) => (
                <div key={card.key} className="flex w-[132px] flex-col items-center gap-2">
                  ...
                  <span
                    className={`text-xs font-bold text-center ${card.isBoss ? "text-red-400" : "text-foreground"} flex items-center justify-center gap-0.5 min-h-[32px]`}
                  >
                  ...
                  <div className="flex w-full flex-col gap-1.5">
                    {card.quests.map((q) => {
                      ...
                      const barrierEl = card.isBoss
                        ? q.barrier?.sub1 || null
                        : getBarrierForSubArea(q, card.subAreaIdx);
                        ...
                        >
                          ...
                          {q.barrier ? (
                            <div className="flex items-center justify-center gap-1 mt-0.5">
                              ...
                              <span
                                className={`text-xs font-bold font-mono ${isDark ? "text-foreground/80" : "text-foreground/70"}`}
                              >
                                {barrierHp}
                              </span>
                            </div>
