...
const getPortalContainer = () => (document.getElementById("portal-root") ?? document.body) as HTMLElement;
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Overlay
    className={cn(
      // fixed → absolute: portal-root(transform 컨텍스트) 기준으로 전체 덮음
...
const sheetVariants = cva(
  // fixed → absolute: portal-root 기준 배치
  "absolute z-50 gap-4 bg-background p-6 shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500",
  {
    variants: {
      side: {
        ...
        "right-wide":
          "inset-y-0 right-0 h-full w-[90vw] max-w-[1400px] border-l data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right",
      },
    },
    defaultVariants: {
      side: "right",
