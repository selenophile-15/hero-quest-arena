...
import { preloadAllEquipmentImages } from "@/lib/equipmentUtils";
...
export default function HeroForm({
  ...
  const maxCommonSlots = heroClass && level ? getMaxCommonSkillSlots(heroClass, Number(level), promoted) : 0;
  ...
  return (
    <div className="animate-fade-in relative">
      ...
      <div className="space-y-4 mt-4" ref={formRef} onKeyDown={handleKeyDown}>
        ...
        <div className={`card-fantasy p-4 ${ringClass}`}>
          <div className="grid grid-cols-[1.5fr_0.8fr_auto_1.5fr_0.7fr_1fr_1fr] gap-3 items-end">
            ...
            <div>
              ...
              <Select
                value={classLine || "_empty"}
                onValueChange={(v) => setClassLine(v === "_empty" ? "" : (v as HeroClassLine))}
              >
                ...
                <SelectContent>
                  ...
                  {jobs.map((j) => (
                    <SelectItem key={j} value={j}>
                      <span
                        className={`inline-flex items-center gap-2 ${heroClass === j ? "font-bold text-primary" : ""}`}
                      >
                        <span>{j}</span>
                      </span>
                    </SelectItem>
              ...
              {(() => {
                const statItems = [
                  ...
                  { icon: STAT_ICON_MAP.crit, value: crit, suffix: " %", baseKey: "totalCrit" as const },
                  {
                    ...
                    suffix: "",
                    isCritDmg: true,
                    baseKey: "totalCritDmg" as const,
                  },
                  {
                    icon: STAT_ICON_MAP.critAttack,
                    value: calcStats?.totalCritAttack ?? critAttack,
                    suffix: "",
                    baseKey: "totalCritAttack" as const,
                  },
                  {
              ...
              <div className="flex flex-col gap-[6px]">
                {ELEMENT_ORDER.map((el) => {
                  ...
                  return (
                    <div
                      key={el.key}
                      className={`flex items-center gap-2 py-0.5 ${isJobElement ? "bg-accent/10 rounded px-1 -mx-1" : ""}`}
                    >
              ...
              {DETAIL_STATS_BASE.map((stat, i) => (
                <div key={i} className="flex items-center justify-between py-0.5 border-b border-border/30 gap-2">
                  ...
                  <span className="text-sm text-foreground tabular-nums text-right w-20">
                    {detailStats[stat]
                      ? stat.includes("%") || stat.includes("계수")
                        ? `${detailStats[stat]}%`
                        : formatNumber(detailStats[stat])
                      : "-"}
                  </span>
                </div>
              ))}
              ...
              {Object.entries(detailStats)
                ...
                .map(([key, val], i) => {
                  ...
                  return (
                    <div
                      key={`cond-${i}`}
                      className="flex items-center justify-between py-0.5 border-b border-border/30 gap-2"
                    >
          ...
          <div className="border border-border rounded overflow-hidden">
            ...
            {Array.from({ length: 4 }).map((_, i) => {
              ...
              const commonThresholds: number[] =
                skillData?.["원소_기준치"]?.map((v: unknown) => Number(v)).filter((v: number) => Number.isFinite(v)) ||
                [];
              ...
              const commonNextThreshold = promoted
                ? (commonThresholds[commonSkillLevelIndex + 1] ?? "-")
                : commonSkillLevelIndex >= 2
                  ? "-"
                  : (commonThresholds[commonSkillLevelIndex + 1] ?? "-");
              ...
              return (
                <div
                  key={i}
                  className={`grid grid-cols-[60px_44px_0.7fr_50px_0.7fr_2fr_100px] gap-0 border-b border-border/30 ${isLocked ? "opacity-30 bg-secondary/10" : ""}`}
                >
                  <div className="px-2 py-1.5 flex items-center justify-center">
                    {grade ? (
        ...
        {/* ─── Row 4: Equipment ─── */}
        <div className="card-fantasy p-4">
          <h3 className="text-sm font-semibold text-primary mb-3" style={{ fontFamily: "'Noto Sans KR', sans-serif" }}>
            장비
          </h3>
          {/* Quiver warning */}
          {equipmentSlots.some((s) => s.item?.type === "quiver") && !hasRanged && (
            <div className="mb-3 p-2 border border-red-400/50 rounded bg-red-400/10">
              <p className="text-sm font-bold text-red-400">
                ⚠ 화살통 경고: 활/크로스보우/총이 장착되지 않아 화살통 스탯이 0으로 처리됩니다.
              </p>
            </div>
          )}
                          ...
                          {equipItem.stats.slice(0, 3).map((stat: any, si: number) => {
                            ...
                            return (
                              <div key={si} className="flex items-center gap-0.5">
                                ...
                                <span
                                  className={`text-sm font-semibold tabular-nums ${isQuiverZero ? "text-red-400 line-through" : "text-foreground"}`}
                                >
          ...
          {equipmentSlots.some((s) => s.item?.relic && s.item?.relicEffect) && (
            <div className="mt-3 p-2 border border-yellow-400/30 rounded bg-yellow-400/5">
              ...
              {equipmentSlots
                .filter((s) => s.item?.relic && s.item?.relicEffect)
                .map((s, i) => (
      ...
      <StatBreakdownDrawer open={breakdownOpen} onOpenChange={setBreakdownOpen} calcStats={calcStats} />
