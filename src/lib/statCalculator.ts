...
export async function calculateHeroStats(input: CalcInput): Promise<CalculatedStats | null> {
  ...
  const equipResult = await calculateEquipmentStats(
    equipmentSlots,
    equipBonuses,
    hasRangedWeapon,
    isSpellknight,
  ...
  // 생각하는 모자 (Thinkin' Cap) 유물: 스킬+영혼 경험치% 합산의 절반을 공통 공격력%에 추가
  const hasThinkinCap = equipmentSlots.some(
    (slot) =>
      slot.item?.id === "mh16_2" || slot.item?.name_ko === "생각하는 모자" || slot.item?.name === "Thinkin' Cap",
  );
  if (hasThinkinCap) {
    const totalExpBonus = bonusSummary.detail.expBonus;
    if (totalExpBonus > 0) {
      const expToAtkPct = totalExpBonus / 2;
      bonusSummary.pctAtk += expToAtkPct;
      bonusSummary.sources.push({
        name: `생각하는 모자 유물 — 경험치% ${totalExpBonus}%의 절반 → 공격력% +${expToAtkPct}%`,
        type: "relic" as const,
        flatAtk: 0,
        flatDef: 0,
        flatHp: 0,
  ...
  if (hasThinkinCap && bonusSummary.detail.expBonus > 0) {
    detailStats["생각하는 모자 - 경험치% 합산"] = bonusSummary.detail.expBonus;
  }
