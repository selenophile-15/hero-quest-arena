...
export async function lookupHeroStats(
  jobName: string,
  level: number,
): Promise<{ level: HeroLevelStats; fixed: HeroFixedStats } | null> {
...
export async function lookupHeroFixedStats(jobName: string): Promise<HeroFixedStats | null> {
  ...
  for (const gyeyeolKey of Object.keys(data)) {
    ...
    for (const groupKey of Object.keys(gyeyeol)) {
      const group = gyeyeol[groupKey];
      const jobData =
        group["기본_직업"]?.["직업명"] === jobName
          ? group["기본_직업"]
          : group["승급_직업"]?.["직업명"] === jobName
            ? group["승급_직업"]
            : null;
      if (jobData) {
...
export async function lookupChampionStats(
  name: string,
  rank: number,
): Promise<{ hp: number; atk: number; def: number; element: number; fixed: HeroFixedStats } | null> {
...
// 앱 진입 시 백그라운드에서 모든 데이터를 미리 로드하는 함수
let _warmupPromise: Promise<void> | null = null;
export function warmupGameData(): void {
