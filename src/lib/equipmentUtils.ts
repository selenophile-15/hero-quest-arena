 ...
 */
export function getEquipImagePath(
  korName: string,
  typeFile: string,
  category: string,
      ...
      try {
        ...
        for (const [tierKey, tierItems] of Object.entries(data)) {
          ...
          for (const [korName, itemData] of Object.entries(tierItems as Record<string, any>)) {
            ...
            const imagePath =
              itemData["이미지_경로"] || (engName ? `/images/equipment/${category}/${file}/${engName}.webp` : "");
            ...
            if (itemData["장비_치명타확률%"])
              stats.push({ key: "장비_치명타확률%", value: itemData["장비_치명타확률%"] });
            ...
            const built: any = {
              ...
              relicEffect:
                typeof itemData["유물"] === "object" && itemData["유물"]?.["효과"] ? itemData["유물"]["효과"] : null,
...
// 앱 시작 시 모든 장비 이미지를 백그라운드에서 미리 로드
let _allEquipPreloadPromise: Promise<void> | null = null;
export async function preloadAllEquipmentImages(): Promise<void> {
  if (_allEquipPreloadPromise) return _allEquipPreloadPromise;
  const nameMap = await loadEquipNameMap();
  const allTypes = Object.keys(EQUIP_TYPE_MAP);
  _allEquipPreloadPromise = loadEquipmentByTypes(allTypes, nameMap).then(() => undefined);
  return _allEquipPreloadPromise;
}
 ...
 */
export function getSlotTypes(
  sid: Record<string, Record<string, string[]>>,
  jobName: string,
  slotIndex: number,
): string[] {
