...
export interface DetailStatsSummary {
  ...
  expBonus: number; // 경험치% 합산 (스킬 + 영혼) — 생각하는 모자 유물용
}
 ...
 */
export function parseSkillBonuses(skills: SkillBonusInput[]): {
  summary: Omit<SkillBonusSummary, "sources">;
  sources: SkillBonusSource[];
} {
  ...
  const detail: DetailStatsSummary = {
    hpRegenPerTurn: 0,
    survivalChance: 0,
    restReduction: 0,
    sharkAtkPct: 0,
  ...
  for (const skill of skills) {
    ...
    for (const [key, rawVal] of Object.entries(skill.bonusData)) {
      ...
      switch (key) {
        ...
        // Detail stats
        case "스킬_매턴체력회복":
          detail.hpRegenPerTurn += val;
          src.regenPerTurn = (src.regenPerTurn || 0) + val;
          break;
        case "스킬_치명타생존%":
          detail.survivalChance += val;
          break;
        case "스킬_휴식시간감소%":
          detail.restReduction += val;
          break;
        case "스킬_체력비례스킬_공격력%":
          detail.berserkerAtkPct += val;
          break;
        case "스킬_체력비례회피%":
          detail.berserkerEvaPct += val;
          break;
        case "스킬_경험치%":
          detail.expBonus += val;
          break;
 ...
 */
export async function parseSoulBonuses(
  souls: SoulBonusInput[],
): Promise<{ summary: Omit<SkillBonusSummary, "sources">; sources: SkillBonusSource[] }> {
    ...
    for (const [key, val] of Object.entries(bonusStats)) {
      ...
      switch (key) {
        ...
        case "영혼_체력%":
          ...
          break;
        case "영혼_깡공격력":
          src.flatAtk += adjusted;
          break;
        case "영혼_깡방어력":
          src.flatDef += adjusted;
          break;
        case "영혼_깡체력":
          src.flatHp += adjusted;
          break;
        case "영혼_치명타확률%":
          src.critRate += adjusted;
          break;
        case "영혼_치명타데미지%":
          src.critDmg += adjusted;
          break;
        case "영혼_회피%":
          src.evasion += adjusted;
          break;
        case "영혼_위협도":
          src.threat += adjusted;
          break;
        ...
        case "영혼_매턴체력회복":
        case "매턴회복":
          detail.hpRegenPerTurn += adjusted;
          src.regenPerTurn = (src.regenPerTurn || 0) + adjusted;
          break;
        case "휴식시간감소%":
          detail.restReduction += adjusted;
          break;
        case "조건부공격력%":
          detail.sharkAtkPct += adjusted;
          break;
        case "영혼_첫라공격력%":
          detail.dinoAtkPct += adjusted;
          break;
        case "영혼_보스공격력%":
          detail.mundraBosPct += adjusted;
          break;
        // 영혼_보스방어력% is same value as mundraBosPct, don't double count
        case "영혼_경험치%":
          detail.expBonus += adjusted;
          break;
      }
    }
...
export function combineBonuses(
  ...
  return {
    ...
    detail: {
      ...
      expBonus: sd.expBonus + sod.expBonus,
    },
